...
![width:1050](images/semantic-tags.svg)
...
![width:1100](images/todo-app-mockup.svg)
...
- 各TODO項目は `<label>` でチェックボックスとテキストを包むと、テキストのクリックでもチェックが切り替わります
...
完成例（HTML部分）は次回スターターの `session03/exercise/todo.html` を参考にしてください。
...
1. `index.html` のGitHubのURL（実習2〜5完了後の状態 = 自己紹介ページ + セマンティックタグ + お問い合わせフォームを含む）
   - 例: `https://github.com/ユーザー名/リポジトリ名/blob/main/session02/exercise/index.html`
2. `todo.html` のGitHubのURL（実習6で作成したTODOアプリのHTML）
